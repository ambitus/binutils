...
cat <<EOF
  ...
  /* z/OS TODO: We can't have any holes in the section/segment map
     because our loader doesn't fill them in. DATA SEGMENT ALIGN
     tends to create holes. Aligning to MAXPAGESIZE with the
     dummy_zos_padding section before it is a clumsy solution that
     creates an extra section and wastes space. Also, it must be on
     the same line as DATA SEGMENT ALIGN because of sed trickery that
     glibc does in it's build process.
     Note that the BYTE forces the section to be marked as having
     contents.  */
  /* Adjust the address for the data segment.  We want to adjust up to
     the same address within the page on the next page up.  */
  .dummy_zos_padding : {BYTE(0x00); . = ALIGN(${MAXPAGESIZE});} ${CREATE_SHLIB-${CREATE_PIE-${RELOCATING+. = ${DATA_ADDR-${DATA_SEGMENT_ALIGN}};}}} ${CREATE_SHLIB+${RELOCATING+. = ${SHLIB_DATA_ADDR-${DATA_SEGMENT_ALIGN}};}} ${CREATE_PIE+${RELOCATING+. = ${SHLIB_DATA_ADDR-${DATA_SEGMENT_ALIGN}};}}
